<template>
    <el-card>
        ...
        >
            ...
            <el-button type="warning" @click="onClearUploadFile" v-else> 重新上传 </el-button>
        </el-upload>
        <div class="flex gap-4 mt-2">
            ...
            <div class="flex flex-col justify-between flex-1">
                <div class="flex gap-1 items-center">
                    <el-input v-model="uploadFileId" placeholder="可直接指定算法侧文件ID" />
                    <el-button plain :icon="Refresh" @click="getFileInfo"></el-button>
                </div>
                <div v-if="uploadFileUrl && faceData.length" class="text-ellipsis line-clamp-3">
                    人脸位置：{{ faceData }}
                </div>
            </div>
        </div>
...
const onError = (err: Error) => {
    ...
    feedback.msgError(
        '上传失败' + (err.name == 'UploadAjaxError' ? '：算法服务连接失败，尝试关闭VPN再试试' : err)
    )
}
...
const getFileInfo = () => {
    const getFileInfoFunc = props.isMaterialFile ? getMaterialFaceList : getUserFaceList
