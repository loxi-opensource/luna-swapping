...
import LunaImageUploader, { type UploadSuccessData } from './LunaImageUploader.vue'
...
const generationLoadingText = computed(() => {
    return generationLoading.value ? '正在处理中...' : '上传用户图，点击【立即生成】'
})
const onGenerate = () => {
        ...
        const pollTask = () => {
            taskStatus({
                ...
                if (response.status === 1) {
                    ...
                    clearInterval(pollTimer)
                    resultFileUrl.value = ossHost + '/' + response.messageList[0].sourceFilePath
    ...
    <div class="edit-popup">
        ...
        >
            <div class="flex gap-4">
                <el-form ref="formRef" :model="formData" label-width="84px" class="w-1/2">
                    <el-form-item
                        label="目标图"
                        prop="targetFileId"
                        required
